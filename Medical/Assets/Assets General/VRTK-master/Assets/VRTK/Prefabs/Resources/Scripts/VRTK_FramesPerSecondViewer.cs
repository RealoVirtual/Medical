            ...
            if (framesTime > updateInterval)
            {
                if (text != null)
                {
                    if (displayFPS)
                    {
						...
						text.text = string.Format("Menor Tiempo De Inyeccion (5 Segundos)", fps);
                    ...
                    {
						text.text = "º Menor tiempo de inyección (5 segundos)\nº Ajuste De Dosis Entre 1 y 80 Unidades";
                    }
                }
                framesCount = 0;
                framesTime = 0;
